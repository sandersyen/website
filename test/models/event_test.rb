...
class EventTest < ActiveSupport::TestCase
  ...
  test 'group with long desc' do
    ...
    assert !Event.new(name: 'Event name', group: @group, description: desc, start_time: Time.now, end_time: Time.now).valid?
  end

  test 'group with bad end time' do
    assert !Event.new(name: 'Event name', group: @group, start_time: Time.now, end_time: Time.now - 10.seconds).valid?
  end
end
